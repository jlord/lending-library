...
// function geoCodeAddresses(gData) {
//   var geoObject = L.mapbox.geocoder("examples.map-20v6611k")
//   var bingGeocoder = new L.Control.BingGeocoder('AlBuFZFSfPMDlEzYaRdy9GvfgBlz8Fl0oZ6Kfwg3e1qOUoTIsih2IdlLr3ROv4bZ')
//   gData.forEach(function(d){
//     var anAddress = d.street + " " + d.city + " " + d.state
//     console.log(anAddress.toString())
//     console.log("bingGeocoder", bingGeocoder)
//     geoObject.query("Oakland, California", returnGeoCode)
//   })

function geoCodeAddresses(gData, callback) {
  var first = "http://dev.virtualearth.net/REST/v1/"
  var second = "?o=json&key="
  var key = "AlBuFZFSfPMDlEzYaRdy9GvfgBlz8Fl0oZ6Kfwg3e1qOUoTIsih2IdlLr3ROv4bZ"
  gData.forEach(function(d){
    var anAddress = d.street + " " + d.city + " " + d.state
    console.log(anAddress)
    // encodeURI(address)
    var url = first + anAddress + second + key
    console.log(url)
    // $.ajaxJSONP({
    //   url: url + '&callback=?',
    //   success: returnGeoCode
    // })
  })
...
function returnGeoCode(err, results) {
  console.log ("err", err, "results", results)
  return console.log(results.latlng)
}
